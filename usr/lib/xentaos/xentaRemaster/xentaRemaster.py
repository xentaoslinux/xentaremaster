...
class Reconstructor:
    ...
    def __init__(self):
        ...
        self.appName = "Xenta Remaster v3.2.1 - Perkakas Remastering Live CD"
        self.codeName = " \"Arok\" "
	...
	if os.path.exists(os.environ['HOME'] + "/.xentaos/xentaRemaster/currentProject"):
		currentProject = commands.getoutput("cat ~/.xentaos/xentaRemaster/currentProject")
	else:
    ...
    def checkWorkingDir(self):
        ...
        rootExists = None
        initrdExists = None
        if os.path.exists(os.path.join(self.customDir, "dvd")) == False:
    ...
    def checkPage(self, pageNum):
            ...
            if self.checkCustomDir() == True:
                if self.checkSetup() == True:
                    if self.checkWorkingDir() == True:
                        ...
                        warnDlg.vbox.pack_start(labelSpc)
                        labelSpc.show()
                        lblContinueText = _("  <b>Lanjutkan Membuat Kerangka Remaster?</b>  ")
                        lblContinueInfo = _("     Ini mungkin memakan waktu beberapa menit. Mohon tunggu...     ")
	    ...
	    self.setPage(self.pageLiveBuild)
            ...
            warnDlg.vbox.pack_start(labelSpc)
            labelSpc.show()
            lblBuildText = _("  <b>Membangun Berkas ISO Live CD dari Proyek Remaster?</b>  ")
            lblBuildInfo = _("     Ini mungkin memakan waktu beberapa menit. Mohon tunggu...     ")
            ...
            if response == gtk.RESPONSE_OK:
                ...
                self.wTree.get_widget("buttonNext").set_label("Selesai")
                return True
            else:
                warnDlg.destroy()
                return False
    ...
    def launchTerminal(self):
	    ...
	    if commands.getoutput('which gnome-terminal') != '':
		print _('Launching gnome-terminal for advanced customization...')
	    	# use x-terminal-emulator if gnome-terminal isn't available
		if os.path.exists("/usr/bin/lxterminal"):
		    	os.popen('export HOME=/root export LC_ALL=C ; lxterminal -e "/tmp/reconstructor-terminal.sh"')
		else:
    ...
    def calculateIsoSize(self):
        try:
            ...
            remaster = commands.getoutput('du -s ' + os.path.join(self.customDir, "dvd/"))
            mRemaster = r.match(remaster)
            remasterSize = int(mRemaster.group(1))
	    print "remasterSize: %d" % remasterSize
            # subtract squashfs root
            if os.path.exists(os.path.join(self.customDir, "dvd/casper/filesystem.squashfs")):
                squash = commands.getoutput('du -s ' + os.path.join(self.customDir, "dvd/casper/filesystem.squashfs"))
    ...
    def saveSetupInfo(self):
        ...
        print _("Info: Sedang Bekerja Membuat Kerangka Remaster. . .")
        self.customDir = self.wTree.get_widget("entryWorkingDir").get_text()
	os.system("mkdir -p ~/.xentaos/xentaRemaster")
	os.system("echo \"" + self.customDir + "\" > ~/.xentaos/xentaRemaster/currentProject")
        ...
        # debug
        print "Kerangka Remaster disesuaikan: " + str(self.customDir)
        print "Membuat Kerangka Proyek Remaster: " + str(self.createRemasterDir)
        print "Membuat Kerangka filesystem Remaster  : " + str(self.createCustomRoot)
        print "Membuat Kerangka Initrd Remaster: " + str(self.createInitrdRoot)
        print "Berkas Image LiveCD ISO: " + str(self.isoFilename)
...
# ---------- Setup ---------- #
    def setupWorkingDirectory(self):
        ...
        if self.createRemasterDir == True:
            ...
            #if os.path.exists(os.path.join(self.customDir, "dvd")):
            #    print _("INFO: Removing existing Remaster directory...")
            #    os.popen('rm -Rf \"' + os.path.join(self.customDir, "dvd/") + '\"')
            if os.path.exists(os.path.join(self.customDir, "dvd")) == False:
                print "INFO: Creating Remaster directory..."
                os.makedirs(os.path.join(self.customDir, "dvd"))
            # check for iso
            if self.isoFilename == "":
            ...
            # copy remaster files
            os.popen('rsync -at --del ' + self.mountDir + '/ \"' + os.path.join(self.customDir, "dvd") + '\"')
	    ...
	    if os.path.exists("/usr/bin/aplay"):
	        os.system("/usr/bin/aplay /usr/lib/xentaos/xentaRemaster/selesai.wav")
...
# ---------- Build ---------- #
    def build(self):
        ...
        self.LiveCdDescription = "Xenta OS x yyy"
        self.LiveCdArch = self.wTree.get_widget("comboboxLiveCdArch").get_active_text()
        self.hfsMap = os.getcwd() + "/lib/hfs.map"

        print " "
        print _("Info: Memulai Membangun Berkas Image ISO dari Kerangka Proyek Remaster...")
        print " "
        # build initrd
        if self.buildInitrd == True:
            # create initrd
            if os.path.exists(os.path.join(self.customDir, "initrd")):
                print _("Membangun initrd dari Kerangka initrd Remaster...")
                #os.popen('cd \"' + os.path.join(self.customDir, "initrd/") + '\"; find | cpio -H newc -o | gzip > ../initrd.gz' + '; mv -f ../initrd.gz \"' + os.path.join(self.customDir, "dvd/casper/initrd.gz") + '\"')
                os.popen('cd \"' + os.path.join(self.customDir, "initrd/") + '\"; find | cpio -H newc -o | lzma -7 > ../initrd.lz' + '; mv -f ../initrd.lz \"' + os.path.join(self.customDir, "dvd/casper/initrd.lz") + '\"')

        # build squash root
        if self.buildSquashRoot == True:
            # create squashfs root
            if os.path.exists(os.path.join(self.customDir, "root")):
                print _("Membangun filesystem.squashfs dari Kerangka filesystem (root) Remaster...")
                print _("Memperbaharui Daftar Paket Paket...")
                q = ' dpkg-query -W --showformat=\'${Package} ${Version}\n\' '
                os.popen('chroot \"' + os.path.join(self.customDir, "root/") + '\"' + q + ' > \"' + os.path.join(self.customDir, "dvd/casper/filesystem.manifest") + '\"' )
                os.popen('cp -f \"' + os.path.join(self.customDir, "dvd/casper/filesystem.manifest") + '\" \"' + os.path.join(self.customDir, "dvd/casper/filesystem.manifest-desktop") + '\"')
                # check for existing squashfs root
                if os.path.exists(os.path.join(self.customDir, "dvd/casper/filesystem.squashfs")):
                    print _("Menghapus filesystem.squashfs lama dari Kerangka Proyek Remaster...")
                    os.popen('rm -Rf \"' + os.path.join(self.customDir, "dvd/casper/filesystem.squashfs") + '\"')
                print _("Membuat filesystem.squashfs baru dari Kerangka Proyek Remaster...")
                # check for alternate mksquashfs
                if mksquashfs == '':
                    os.system(self.timeCmd + ' mksquashfs \"' + os.path.join(self.customDir, "root/") + '\"' + ' \"' + os.path.join(self.customDir, "dvd/casper/filesystem.squashfs") + '\"')
                else:
                    os.system(self.timeCmd + ' ' + mksquashfs + ' \"' + os.path.join(self.customDir, "root/") + '\"' + ' \"' + os.path.join(self.customDir, "dvd/casper/filesystem.squashfs") + '\"')
        ...
        if self.buildIso == True:
            # create iso
            if os.path.exists(os.path.join(self.customDir, "dvd")):
                print _("Membuat berkas image iso dari Kerangka Proyek Remaster...")
                # add disc id
		...
		os.system("/usr/lib/xentaos/xentaRemaster/updateManifest.sh " + self.customDir)
                # update md5
                print _("Memperbaharui md5sums...")
                os.system('rm ' + os.path.join(self.customDir, "dvd/") + ' md5sum.txt')
                os.popen('cd \"' + os.path.join(self.customDir, "dvd/") + '\"; ' + 'find . -type f -print0 | xargs -0 md5sum > md5sum.txt')
		#Remove md5sum.txt from md5sum.txt
		os.system("sed -e '/md5sum.txt/d' " + os.path.join(self.customDir, "dvd/") + "md5sum.txt > " + os.path.join(self.customDir, "dvd/") + "md5sum.new")
		os.system("mv " + os.path.join(self.customDir, "dvd/") + "md5sum.new " + os.path.join(self.customDir, "dvd/") + "md5sum.txt")
		#Remove boot.cat from md5sum.txt
		os.system("sed -e '/boot.cat/d' " + os.path.join(self.customDir, "dvd/") + "md5sum.txt > " + os.path.join(self.customDir, "dvd/") + "md5sum.new")
		os.system("mv " + os.path.join(self.customDir, "dvd/") + "md5sum.new " + os.path.join(self.customDir, "dvd/") + "md5sum.txt")
		#Remove isolinux.bin from md5sum.txt
		os.system("sed -e '/isolinux.bin/d' " + os.path.join(self.customDir, "dvd/") + "md5sum.txt > " + os.path.join(self.customDir, "dvd/") + "md5sum.new")
		os.system("mv " + os.path.join(self.customDir, "dvd/") + "md5sum.new " + os.path.join(self.customDir, "dvd/") + "md5sum.txt")
                # remove existing iso
                if os.path.exists(self.buildLiveCdFilename):
                    print _("Menghapus berkas image iso lama dari Kerangka Proyek Remaster...")
                ...
                print _("Berkas image iso arsitektur x86 / 32bit...")
                os.popen(self.timeCmd + ' mkisofs -o \"' + self.buildLiveCdFilename + '\" -b \"isolinux/isolinux.bin\" -c \"isolinux/boot.cat\" -no-emul-boot -boot-load-size 4 -boot-info-table -V \"' + self.LiveCdDescription + '\" -cache-inodes -r -J -l \"' + os.path.join(self.customDir, "dvd") + '\"')
                #elif self.LiveCdArch == "PowerPC":
                #    print _("Building PowerPC ISO...")
                #    os.popen(self.timeCmd + ' mkisofs  -r -V \"' + self.LiveCdDescription + '\" --netatalk -hfs -probe -map \"' + self.hfsMap + '\" -chrp-boot -iso-level 2 -part -no-desktop -hfs-bless ' + '\"' + os.path.join(self.customDir, "remaster/install") + '\" -o \"' + self.buildLiveCdFilename + '\" \"' + os.path.join(self.customDir, "dvd") + '\"')
                #elif self.LiveCdArch == "x86_64":
                 #   print _("Building x86_64 ISO...")
                  #  os.popen(self.timeCmd + ' mkisofs -r -o \"' + self.buildLiveCdFilename + '\" -b \"isolinux/isolinux.bin\" -c \"isolinux/boot.cat\" -no-emul-boot -V \"' + self.LiveCdDescription + '\" -J -l \"' + os.path.join(self.customDir, "dvd") + '\"')

        self.setDefaultCursor()
        self.setPage(self.pageFinish)
        # print status message
        statusMsgFinish = _('     <b>Selesai.</b>     ')
        statusMsgISO = _('      <b>Selesai.</b>  Lokasi Berkas Image ISO di: ')
        if os.path.exists(self.buildLiveCdFilename):
        ...
        print "Membangun Proyek Selesai..."
	if os.path.exists("/usr/bin/aplay"):
		os.system("/usr/bin/aplay /usr/lib/xentaos/xentaRemaster/selesai.wav")
...
if __name__ == "__main__":
    ...
    if os.getuid() != 0 :
        ...
        warnDlg = gtk.Dialog(title="xentaRemaster", parent=None, flags=0, buttons=(gtk.STOCK_OK, gtk.RESPONSE_OK))
        warnDlg.set_icon_from_file('/usr/lib/xentaos/xentaRemaster/icon.png')
        warnDlg.vbox.set_spacing(10)
        labelSpc = gtk.Label(" ")
        warnDlg.vbox.pack_start(labelSpc)
        labelSpc.show()
